...
export async function searchHospital(req, res, next) {
  try {
    ...
    // Create a clean search params object without userId
    const searchParamsForHistory = { ...searchParams };
    delete searchParamsForHistory.userId;

    const lastquery = constructLastquery(searchParamsForHistory);
    ...
    res
      ...
      .json({ data: searchData, lastSearch: lastSearchDoc?.lastquery });
  } catch (error) {
